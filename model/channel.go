...
func (channel *Channel) GetEnabledKeyByIndex(index int) (string, int, bool, *types.NewAPIError) {
	if channel == nil {
		return "", 0, false, nil
	}
	if !channel.ChannelInfo.IsMultiKey {
		if index == 0 && channel.Key != "" {
			return channel.Key, 0, true, nil
		}
		return "", 0, false, nil
	}
	if index < 0 {
	...
	if !channel.multiKeyIndexEnabledLocked(index) {
		return "", 0, false, nil
	}
	return keys[index], index, true, nil
}

func (channel *Channel) GetStableEnabledKey(seed string) (string, int, bool, *types.NewAPIError) {
	if channel == nil || strings.TrimSpace(seed) == "" {
		return "", 0, false, nil
	}
	if !channel.ChannelInfo.IsMultiKey {
		if channel.Key != "" {
	...
	enabledIdx := channel.enabledMultiKeyIndexesLocked(keys)
	if len(enabledIdx) == 0 {
		return "", 0, false, types.NewError(errors.New("no enabled keys"), types.ErrorCodeChannelNoAvailableKey)
	}
	h := fnv.New32a()
	_, _ = h.Write([]byte(seed))
	selectedIdx := enabledIdx[int(h.Sum32())%len(enabledIdx)]
	return keys[selectedIdx], selectedIdx, true, nil
}

func (channel *Channel) multiKeyIndexEnabledLocked(index int) bool {
...
func (channel *Channel) enabledMultiKeyIndexesLocked(keys []string) []int {
	enabledIdx := make([]int, 0, len(keys))
	for i := range keys {
		if channel.multiKeyIndexEnabledLocked(i) {
			enabledIdx = append(enabledIdx, i)
		}
	}
	return enabledIdx
...
func (channel *Channel) AllowAutoTestAndRecover() bool {
	if channel == nil {
		return false
	}
	return !channel.GetOtherSettings().AutoTestAndRecoverDisabled
}
...
func MarkChannelAntiPoisonRisk(channelId int, usingKey string, reason string) bool {
	reason = strings.TrimSpace(reason)
	if reason == "" {
		reason = "anti-poison validation failed"
	}
	statusReason := "anti-poison risk: " + reason
	...
	if common.MemoryCacheEnabled {
		...
		if channelCache != nil {
			if channelCache.ChannelInfo.IsMultiKey {
				pollingLock := GetChannelPollingLock(channelId)
				pollingLock.Lock()
				handlerMultiKeyUpdate(channelCache, usingKey, common.ChannelStatusManuallyDisabled, statusReason)
				channelCache.Status = common.ChannelStatusManuallyDisabled
				pollingLock.Unlock()
			} else {
				channelCache.Status = common.ChannelStatusManuallyDisabled
			}
			CacheUpdateChannelStatus(channelId, common.ChannelStatusManuallyDisabled)
		}
	...
	if channel.ChannelInfo.IsMultiKey {
		...
		pollingLock := GetChannelPollingLock(channelId)
		pollingLock.Lock()
		handlerMultiKeyUpdate(channel, usingKey, common.ChannelStatusManuallyDisabled, statusReason)
	...
	info["risk_status"] = "anti_poison"
	info["risk_reason"] = reason
	info["risk_time"] = now
	info["risk_action"] = "manual_disabled"
	channel.SetOtherInfo(info)

	if err := channel.SaveWithoutKey(); err != nil {
		common.SysLog(fmt.Sprintf("failed to mark channel anti-poison risk: channel_id=%d, error=%v", channel.Id, err))
		return false
	}
...
func SetChannelAntiPoisonEvidencePath(channelId int, evidencePath string) error {
	...
	info := channel.GetOtherInfo()
	info["risk_evidence_path"] = evidencePath
	info["risk_evidence_time"] = common.GetTimestamp()
	channel.SetOtherInfo(info)
	if err := channel.SaveWithoutKey(); err != nil {
		return err
	}
...
func ClearChannelAntiPoisonRisk(channelId int, enable bool) error {
	channel, err := GetChannelById(channelId, true)
	if err != nil {
		return err
	}
	...
	delete(info, "risk_status")
	delete(info, "risk_reason")
	delete(info, "risk_time")
	delete(info, "risk_action")
	delete(info, "risk_evidence_path")
	delete(info, "risk_evidence_time")
	if strings.HasPrefix(statusReason, "anti-poison risk:") {
	...
	if enable {
		if err := UpdateAbilityStatus(channelId, true); err != nil {
			common.SysLog(fmt.Sprintf("failed to update ability status after clearing anti-poison risk: channel_id=%d, error=%v", channelId, err))
		}
	}
	CacheUpdateChannel(channel)
	return nil
