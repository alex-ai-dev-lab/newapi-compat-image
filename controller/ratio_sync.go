...
func isOfficialModelsDevProvider(provider string) bool {
	return officialModelsDevProviderIDs[strings.ToLower(strings.TrimSpace(provider))]
}

func isOfficialModelsDevModelName(modelName string) bool {
	modelName = strings.TrimSpace(modelName)
	if modelName == "" {
		return false
	}
	// models.dev can include reseller/aggregator aliases even under otherwise
	// official provider buckets, for example "siliconflow/deepseek-v3". Ken
	// wants official model IDs only, so do not import slash-prefixed aliases.
...
func convertModelsDevToRatioData(reader io.Reader) (map[string]any, error) {
	...
	for _, provider := range providers {
		if !isOfficialModelsDevProvider(provider) {
			continue
		}
