...
func TestChannelAffinityMultiKeyIndexCacheUsesRawKey(t *testing.T) {
	cacheKeySuffix := "test-rule:default:test-fp"
	cacheKeyFull := channelAffinityCacheNamespace + ":" + cacheKeySuffix
	idxCache := getChannelAffinityMultiKeyIndexCache()
	_, _ = idxCache.DeleteMany([]string{cacheKeySuffix})
	t.Cleanup(func() {
		_, _ = idxCache.DeleteMany([]string{cacheKeySuffix})
	})
	...
	RecordChannelAffinity(ctx, 75)
	idx, found := getChannelAffinityPreferredMultiKeyIndex(cacheKeySuffix)
	require.True(t, found)
	require.Equal(t, 2, idx)

	idx, found = getChannelAffinityPreferredMultiKeyIndex(cacheKeyFull)
	require.True(t, found)
	require.Equal(t, 2, idx)
}

func TestResolveChannelAffinityMultiKeyFallsBackToStableEnabledKey(t *testing.T) {
	rec := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(rec)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/v1/messages", nil)
	setChannelAffinityContext(ctx, channelAffinityMeta{
		CacheKey:       channelAffinityCacheNamespace + ":test-rule:default:test-fp",
		TTLSeconds:     60,
		RuleName:       "test-rule",
		KeyFingerprint: "test-fp",
	})
	...
	key1, index1, used1, err1 := ResolveChannelAffinityMultiKey(ctx, ch)
	require.Nil(t, err1)
	require.True(t, used1)
	require.NotEqual(t, 1, index1)
	require.NotEmpty(t, key1)

	key2, index2, used2, err2 := ResolveChannelAffinityMultiKey(ctx, ch)
	require.Nil(t, err2)
	require.True(t, used2)
	require.Equal(t, index1, index2)
	require.Equal(t, key1, key2)
}

func TestResolveChannelAffinityMultiKeyUsesCachedPreferredIndex(t *testing.T) {
	rec := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(rec)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/v1/messages", nil)
	...
	key, index, used, err := ResolveChannelAffinityMultiKey(ctx, ch)
	require.Nil(t, err)
	require.True(t, used)
