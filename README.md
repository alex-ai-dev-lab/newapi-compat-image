...
- OpenAI 兼容上游默认改写为 Codex CLI 风格 `User-Agent`，减少被 `Go-http-client/1.1` 识别拦截的概率。

### 4. Claude Messages 兼容头处理

- Claude 类上游默认改写为 Claude CLI 风格 `User-Agent`。
- 只在平台没有显式覆盖 `User-Agent` 时注入，保留渠道自定义请求头优先级。
