...
| 16 | 每个模块尽量能后台配置，尤其配色、字体、内容宽度等外观项 | 部分实现 | `setting/system_setting/theme.go`、`controller/misc.go`、`controller/option.go`、`theme-customization-provider.tsx`、`appearance-settings.tsx`、`system-info-section.tsx`、`system-config-store.ts`、`dashboard-section.tsx`、`sidebar-modules-section.tsx`、`settings-navigation-section.tsx`、`use-sidebar-config.ts` | 后台全局默认值只在用户没有本地偏好 cookie 时生效；顶部主题切换仍作为个人偏好入口。System Information、Appearance、Data Dashboard 默认项、侧边栏模块、System Settings navigation 和 Header navigation 支持 JSON 导入/导出；侧边栏模块支持全局显示/隐藏、顶层分组排序、分组内排序；System Settings navigation 支持系统设置大区/子项显隐排序；尚未做到“每个模块的所有参数均后台可配置” |
