...
本轮 Dashboard 默认项增量：`系统设置 -> 内容/外观 -> Data Dashboard` 已补充统计默认时间范围、默认自动刷新开关、默认刷新间隔、默认表格页大小、默认健康筛选和默认趋势模式配置；后端 option 会通过 `/api/status` 下发到前端，Dashboard 在浏览器没有保存本地偏好时使用后台默认值，用户手动切换后的本地偏好仍优先；`Reset view` 会回到后台默认视角，而不是固定写死 25 行 / all / overview。
