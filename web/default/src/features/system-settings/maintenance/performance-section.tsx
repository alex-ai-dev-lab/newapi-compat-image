...
type PerformanceImportExportPayload = {
  Performance?: Partial<FlatPerfDefaults>
} & Partial<FlatPerfDefaults>
...
export function PerformanceSection(props: Props) {
  ...
  const currentNormalizedValues = () =>
    normalizeFormValues(perfSchema.parse(form.getValues()))
  ...
  const exportConfig = async () => {
    ...
    try {
      ...
      toast.success(t('Performance JSON exported and copied'))
    } catch {
      toast.success(t('Performance JSON exported'))
    }
  }
  ...
  const openImportDialog = () => {
    setImportText(
      JSON.stringify(
        {
          Performance: currentNormalizedValues(),
        },
        null,
        2
      )
  ...
  const importConfig = async () => {
    try {
      const raw = JSON.parse(importText) as PerformanceImportExportPayload
      const source = raw.Performance ?? raw
      const next = {
        ...currentNormalizedValues(),
        ...source,
      } as FlatPerfDefaults
      const parsed = perfSchema.parse(buildFormDefaults(next))

      form.setValue('performance_setting', parsed.performance_setting, {
        shouldDirty: true,
        shouldValidate: true,
      })
      form.setValue('perf_metrics_setting', parsed.perf_metrics_setting, {
        shouldDirty: true,
        shouldValidate: true,
      })
      await form.trigger()
      setImportOpen(false)
      toast.success(t('Performance settings imported. Click Save to apply.'))
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : t('Invalid performance JSON')
      )
    }
  }
    ...
    <SettingsSection title={t('Performance Settings')}>
      ...
      <Dialog open={importOpen} onOpenChange={setImportOpen}>
        <DialogContent className='max-w-2xl'>
          <DialogHeader>
            <DialogTitle>{t('Import performance JSON')}</DialogTitle>
            <DialogDescription>
              {t(
                'Paste an exported performance JSON payload. Imported values stay local until you save settings.'
              )}
            </DialogDescription>
          </DialogHeader>
          <Textarea
