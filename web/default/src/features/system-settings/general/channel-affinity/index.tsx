...
type ChannelAffinityPayload = Partial<ChannelAffinitySettings> & {
  ChannelAffinity?: Partial<ChannelAffinitySettings>
  channel_affinity_setting?: Partial<{
    enabled: boolean
    switch_on_success: boolean
    max_entries: number
    default_ttl_seconds: number
    rules: string | AffinityRule[]
  }>
}
...
export function ChannelAffinitySection(props: Props) {
  ...
  const getCurrentRulesJson = () => {
    if (editMode !== 'json') return serializeRules(rules)
    try {
      const parsed = JSON.parse(jsonText)
  ...
  const exportConfig = async () => {
    const payload: ChannelAffinitySettings = {
      'channel_affinity_setting.enabled': enabled,
      'channel_affinity_setting.switch_on_success': switchOnSuccess,
      'channel_affinity_setting.max_entries': maxEntries,
      'channel_affinity_setting.default_ttl_seconds': defaultTtl,
      'channel_affinity_setting.rules': getCurrentRulesJson(),
    }
  ...
  const normalizeImportedRules = (value: unknown): AffinityRule[] => {
  ...
  const importConfig = () => {
    try {
      const raw = JSON.parse(importText) as ChannelAffinityPayload
      const payload = (raw.ChannelAffinity ??
        raw) as Partial<ChannelAffinitySettings>
      const nested = raw.channel_affinity_setting
      const importedRules = normalizeImportedRules(
        payload['channel_affinity_setting.rules'] ?? nested?.rules
      )

      setEnabled(
        payload['channel_affinity_setting.enabled'] ??
          nested?.enabled ??
          enabled
      )
      setSwitchOnSuccess(
        payload['channel_affinity_setting.switch_on_success'] ??
          nested?.switch_on_success ??
          switchOnSuccess
      )
      setMaxEntries(
        Number(
          payload['channel_affinity_setting.max_entries'] ??
            nested?.max_entries ??
            maxEntries
        )
      )
      setDefaultTtl(
        Number(
          payload['channel_affinity_setting.default_ttl_seconds'] ??
            nested?.default_ttl_seconds ??
            defaultTtl
        )
      )
      ...
      setJsonText(
        JSON.stringify(
          importedRules.map(({ id: _, ...rule }) => rule),
          null,
          2
        )
      )
      setImportOpen(false)
      toast.success(t('Channel affinity imported'))
    } catch {
      toast.error(t('Invalid channel affinity JSON'))
    }
  }
  ...
  return (
    <>
      ...
      <Dialog open={importOpen} onOpenChange={setImportOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('Import channel affinity')}</DialogTitle>
          </DialogHeader>
          <Textarea
            value={importText}
            onChange={(event) => setImportText(event.target.value)}
            placeholder={t('Paste channel affinity JSON')}
