...
type ApiInfoImportExportPayload = {
  ApiAddresses?: {
    enabled?: boolean
    items?: unknown
  }
  ...
  'console_setting.api_info'?: unknown
}
...
export function ApiInfoSection({ enabled, data }: ApiInfoSectionProps) {
  ...
  const normalizeApiInfoList = (value: unknown): ApiInfo[] => {
    const raw = typeof value === 'string' ? JSON.parse(value || '[]') : value
    if (!Array.isArray(raw)) {
      throw new Error('API addresses must be an array')
    }

    return raw.map((item, idx) => {
      const parsed = apiInfoSchema.parse(item)
      return {
        id:
          typeof item === 'object' &&
          item !== null &&
  ...
  const exportConfig = async () => {
    const payload = {
      ...
      'console_setting.api_info_enabled': isEnabled,
      'console_setting.api_info': apiInfoList,
    }
    ...
    try {
      ...
      toast.success(t('API addresses JSON exported and copied'))
    } catch {
      toast.success(t('API addresses JSON exported'))
    }
  }
  ...
  const importConfig = () => {
    try {
      const raw = JSON.parse(importText) as ApiInfoImportExportPayload
      const source = raw.ApiAddresses ?? raw
      const importedItems =
        source.items ?? raw.apiInfo ?? raw['console_setting.api_info']
      const importedEnabled =
        typeof source.enabled === 'boolean'
          ? source.enabled
          : raw['console_setting.api_info_enabled']

      if (importedItems === undefined) {
        throw new Error('Missing API addresses')
      }

      const nextApiInfoList = normalizeApiInfoList(importedItems)
      setApiInfoList(nextApiInfoList)
      setSelectedIds([])
      ...
      setHasChanges(true)
      setImportOpen(false)
      toast.success(t('API addresses imported. Click Save Settings to apply.'))
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : t('Invalid API addresses JSON')
      )
    }
  }
    ...
    <SettingsSection title={t('API Addresses')}>
      ...
      <Dialog open={importOpen} onOpenChange={setImportOpen}>
        <DialogContent className='max-w-2xl'>
          <DialogHeader>
            <DialogTitle>{t('Import API addresses JSON')}</DialogTitle>
            <DialogDescription>
              {t(
                'Paste an exported API addresses JSON payload. Imported values stay local until you save settings.'
              )}
            </DialogDescription>
          </DialogHeader>
          <Textarea
