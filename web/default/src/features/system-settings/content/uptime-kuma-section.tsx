...
type UptimeKumaImportExportPayload = {
  UptimeKuma?: {
    enabled?: boolean
    groups?: unknown
  }
  ...
  'console_setting.uptime_kuma_groups'?: unknown
}
...
export function UptimeKumaSection({ enabled, data }: UptimeKumaSectionProps) {
  ...
  const normalizeGroups = (value: unknown): UptimeKumaGroup[] => {
    const raw = typeof value === 'string' ? JSON.parse(value || '[]') : value
    if (!Array.isArray(raw)) {
      throw new Error('Uptime Kuma groups must be an array')
    }

    return raw.map((item, idx) => {
      const parsed = uptimeKumaSchema.parse(item)
      return {
        id:
          typeof item === 'object' &&
          item !== null &&
  ...
  const exportConfig = async () => {
    const payload = {
      ...
      'console_setting.uptime_kuma_enabled': isEnabled,
      'console_setting.uptime_kuma_groups': groups,
    }
    ...
    try {
      ...
      toast.success(t('Uptime Kuma JSON exported and copied'))
    } catch {
      toast.success(t('Uptime Kuma JSON exported'))
    }
  }
  ...
  const importConfig = () => {
    try {
      const raw = JSON.parse(importText) as UptimeKumaImportExportPayload
      const source = raw.UptimeKuma ?? raw
      const importedGroups =
        source.groups ??
        raw.uptimeKuma ??
        raw['console_setting.uptime_kuma_groups']
      const importedEnabled =
        typeof source.enabled === 'boolean'
          ? source.enabled
          : raw['console_setting.uptime_kuma_enabled']

      if (importedGroups === undefined) {
        throw new Error('Missing Uptime Kuma groups')
      }

      const nextGroups = normalizeGroups(importedGroups)
      ...
      setHasChanges(true)
      setImportOpen(false)
      toast.success(t('Uptime Kuma imported. Click Save Settings to apply.'))
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : t('Invalid Uptime Kuma JSON')
      )
    }
  }
    ...
    <SettingsSection title={t('Uptime Kuma')}>
      ...
      <Dialog open={importOpen} onOpenChange={setImportOpen}>
        <DialogContent className='max-w-2xl'>
          <DialogHeader>
            <DialogTitle>{t('Import Uptime Kuma JSON')}</DialogTitle>
            <DialogDescription>
              {t(
                'Paste an exported Uptime Kuma JSON payload. Imported values stay local until you save settings.'
              )}
            </DialogDescription>
          </DialogHeader>
          <Textarea
