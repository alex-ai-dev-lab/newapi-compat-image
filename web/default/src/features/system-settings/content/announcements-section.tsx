...
type AnnouncementsImportExportPayload = {
  ...
  'console_setting.announcements'?: unknown
}
...
export function AnnouncementsSection({
  ...
  const normalizeAnnouncements = (value: unknown): Announcement[] => {
    ...
    return raw.map((item, idx) => {
      const parsed = announcementSchema.parse(item)
      return {
        id:
          typeof item === 'object' &&
          item !== null &&
  ...
  const exportConfig = async () => {
    const payload = {
      ...
      'console_setting.announcements_enabled': isEnabled,
    ...
    try {
      ...
      toast.success(t('Announcements JSON exported and copied'))
    } catch {
      toast.success(t('Announcements JSON exported'))
    }
  }
  ...
  const importConfig = () => {
    try {
      const raw = JSON.parse(importText) as AnnouncementsImportExportPayload
      const source = raw.ConsoleAnnouncements ?? raw
      const importedAnnouncements =
        source.announcements ?? raw['console_setting.announcements']
      const importedEnabled =
        typeof source.enabled === 'boolean'
          ? source.enabled
          : raw['console_setting.announcements_enabled']

      if (importedAnnouncements === undefined) {
        throw new Error('Missing announcements')
      }

      const nextAnnouncements = normalizeAnnouncements(importedAnnouncements)
      ...
      setHasChanges(true)
      setImportOpen(false)
      toast.success(t('Announcements imported. Click Save Settings to apply.'))
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : t('Invalid announcements JSON')
      )
    }
  }
  ...
  return (
    <SettingsSection title={t('Announcements')}>
      ...
      <Dialog open={importOpen} onOpenChange={setImportOpen}>
        <DialogContent className='max-w-2xl'>
          <DialogHeader>
            <DialogTitle>{t('Import announcements JSON')}</DialogTitle>
            <DialogDescription>
              {t(
                'Paste an exported announcements JSON payload. Imported values stay local until you save settings.'
              )}
            </DialogDescription>
          </DialogHeader>
          <Textarea
