...
import { Code2, Download, Eye, ShieldAlert, Upload } from 'lucide-react'
...
type PaymentImportExportPayload = {
  PaymentGateway?: Partial<PaymentFormValues> & PaymentImportExtraValues
  paymentGateway?: Partial<PaymentFormValues> & PaymentImportExtraValues
} & Partial<PaymentFormValues> &
  PaymentImportExtraValues
type PaymentImportExtraValues = {
  WaffoPayMethods?: PayMethod[] | string
...
] as const satisfies ReadonlyArray<keyof PaymentFormValues>

const PAYMENT_JSON_FIELD_KEYS = [
  'PayMethods',
  'AmountOptions',
  'AmountDiscount',
  'CreemProducts',
...
const PAYMENT_SECRET_KEY_SET = new Set<keyof PaymentFormValues>(
  PAYMENT_SECRET_KEYS
)
const PAYMENT_JSON_FIELD_KEY_SET = new Set<keyof PaymentFormValues>(
  PAYMENT_JSON_FIELD_KEYS
)
...
function parseImportedWaffoPayMethods(value: unknown): PayMethod[] | undefined {
  if (value === undefined) return undefined
  if (Array.isArray(value)) return value as PayMethod[]
  if (typeof value === 'string') return parseWaffoPayMethods(value)
  return undefined
}

function importSecret(current: string, incoming: unknown) {
  if (typeof incoming !== 'string') return current
  const trimmed = incoming.trim()
  if (!trimmed || trimmed === REDACTED_SECRET) return current
  return trimmed
}

function redactSecret(value: unknown) {
  return typeof value === 'string' && value.trim() ? REDACTED_SECRET : ''
}

function normalizeJsonImportValue(value: unknown, current: string) {
  if (value === undefined) return current
  if (typeof value === 'string') return formatJsonForEditor(value)
...
export function PaymentSettingsSection({
  ...
  const normalizedValues = (): PaymentFormValues => {
    const values = paymentSchema.parse(form.getValues())
    return {
      ...values,
      PayAddress: removeTrailingSlash(values.PayAddress),
      EpayId: values.EpayId.trim(),
      ...
      WaffoCurrency: values.WaffoCurrency.trim() || 'USD',
      WaffoNotifyUrl: values.WaffoNotifyUrl.trim(),
      WaffoReturnUrl: removeTrailingSlash(values.WaffoReturnUrl.trim()),
      WaffoPancakeMerchantID: values.WaffoPancakeMerchantID.trim(),
      WaffoPancakePrivateKey: values.WaffoPancakePrivateKey.trim(),
      WaffoPancakeReturnURL: removeTrailingSlash(
        values.WaffoPancakeReturnURL.trim()
  ...
  const buildExportValues = () => {
    ...
    PAYMENT_SECRET_KEYS.forEach((key) => {
      safeValues[key] = redactSecret(values[key])
    })
  ...
  const exportConfig = async () => {
    const safeValues = buildExportValues()
    const payload = {
      PaymentGateway: safeValues,
      ...safeValues,
    }
    ...
    try {
      ...
      toast.success(t('Payment Gateway JSON exported and copied'))
    } catch {
      toast.success(t('Payment Gateway JSON exported'))
    }
  }
  ...
  const openImportDialog = () => {
    setImportText(
      JSON.stringify(
        {
          PaymentGateway: buildExportValues(),
        },
        null,
        2
      )
  ...
  const importConfig = async () => {
    try {
      const raw = JSON.parse(importText) as PaymentImportExportPayload
      const source = raw.PaymentGateway ?? raw.paymentGateway ?? raw
      const current = normalizedValues()
      const merged: Record<string, unknown> = { ...current }

      PAYMENT_FIELD_KEYS.forEach((key) => {
        const incoming = source[key]
        if (PAYMENT_SECRET_KEY_SET.has(key)) {
          merged[key] = importSecret(current[key] as string, incoming)
          return
        }
        if (PAYMENT_JSON_FIELD_KEY_SET.has(key)) {
          merged[key] = normalizeJsonImportValue(
            incoming,
      ...
      const parsed = paymentSchema.parse(merged)
      PAYMENT_FIELD_KEYS.forEach((key) => {
      ...
      const importedPayMethods = parseImportedWaffoPayMethods(
        source.WaffoPayMethods
      )
      if (importedPayMethods) {
        setWaffoPayMethods(importedPayMethods)
      }

      const importedStoreID =
        source.WaffoPancakeStoreID ?? source.waffoPancakeStoreID
      const importedProductID =
        source.WaffoPancakeProductID ?? source.waffoPancakeProductID
      if (
        typeof importedStoreID === 'string' ||
        typeof importedProductID === 'string'
      ) {
        setWaffoPancakeSelection({
          storeID:
            typeof importedStoreID === 'string'
              ? importedStoreID.trim()
              : waffoPancakeSelection.storeID,
          productID:
            typeof importedProductID === 'string'
              ? importedProductID.trim()
      ...
      await form.trigger()
      setImportOpen(false)
      toast.success(t('Payment Gateway imported. Click Save to apply.'))
    } catch (error) {
      toast.error(
        error instanceof Error
          ? error.message
          : t('Invalid Payment Gateway JSON')
      )
    }
  }
    ...
    <SettingsSection title={t('Payment Gateway')}>
                  ...
                  <FormControl>
                    {payMethodsVisualMode ? (
                      ...
                      <Textarea
                        rows={4}
                        placeholder={t(
                          '[{"name":"支付宝","type":"alipay","color":"var(--chart-1)"}]'
                        )}
                        {...field}
      ...
      <Dialog open={importOpen} onOpenChange={setImportOpen}>
        <DialogContent className='sm:max-w-3xl'>
          <DialogHeader>
            <DialogTitle>{t('Import Payment Gateway JSON')}</DialogTitle>
            <DialogDescription>
              {t(
                'Paste an exported payment gateway JSON payload. Redacted or empty credentials keep the current saved secret; imported values are not saved until you click Save.'
              )}
            </DialogDescription>
          </DialogHeader>
          <Textarea
