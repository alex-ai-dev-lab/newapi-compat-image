...
export const channelFormSchema = z
  .object({
    ...
    auto_test_time_window_start: z.string().optional(),
    auto_test_time_window_end: z.string().optional(),
    auto_test_timezone: z.string().optional(),
    ...
    auto_test_and_recover_enabled: z.boolean().optional(),
...
type ClaudeThinkingSupportMode = NonNullable<
  ChannelFormValues['supports_claude_thinking']
>
...
export const CHANNEL_FORM_DEFAULT_VALUES: ChannelFormValues = {
  ...
  auto_test_retry_threshold: 2,
  auto_test_time_window_start: '08:00',
  auto_test_time_window_end: '18:00',
  auto_test_timezone: 'Asia/Taipei',
  ...
  auto_test_and_recover_enabled: true,
...
export function transformChannelToFormDefaults(
  ...
  if (channel.setting) {
    try {
      ...
      extraSettings = {
        ...
        requires_codex_identity:
          typeof parsed.requires_codex_identity === 'boolean'
            ? (String(parsed.requires_codex_identity) as CodexIdentityMode)
            : 'auto',
        supports_claude_thinking:
  ...
  let autoTestAndRecoverEnabled = true
  ...
  return {
    ...
    auto_test_and_recover_enabled: autoTestAndRecoverEnabled,
...
function buildSettingJSON(formData: ChannelFormValues): string {
  ...
  if (formData.supports_claude_thinking === 'true') {
    settingObj.supports_claude_thinking = true
  } else if (formData.supports_claude_thinking === 'false') {
    settingObj.supports_claude_thinking = false
  }
  if (formData.requires_codex_identity === 'true') {
    settingObj.requires_codex_identity = true
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  settingsObj.auto_test_and_recover_disabled =
    formData.auto_test_and_recover_enabled === false
