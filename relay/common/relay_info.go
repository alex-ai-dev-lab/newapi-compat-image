...
type RelayInfo struct {
	...
	AntiPoisonGuardPrefix string
	// AntiPoisonResponseProofNonce is a per-request nonce that normal text
	// responses must echo in a hidden proof marker before any text is released
	// downstream. It is independent from the tool-call guard prefix.
	AntiPoisonResponseProofNonce string
	// AntiPoisonCanaryNonce is a per-request nonce injected into the last user
	// message that the model must echo at the end of its reply. This catches
	// 200-OK ad payloads that cannot see per-request user message content.
	AntiPoisonCanaryNonce string
