...
func stripOpenAIStreamResponseProof(data string, proof *antipoison.ProofStreamValidator) (string, bool, error) {
	if proof == nil || proof.Verified() || data == "" {
	...
	for i := range chunk.Choices {
		...
		cleaned, hold, err := proof.ProcessText(delta.GetContentString())
		if err != nil {
			return "", true, err
		}
		if hold {
...
func openAIStreamChunkHasVisibleText(data string) bool {
	if data == "" {
		return false
	}
	var chunk dto.ChatCompletionsStreamResponse
...
func OaiStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	helper.StreamScannerHandler(c, resp, info, func(data string, sr *helper.StreamResult) {
		if lastStreamData != "" {
			if canaryBuffer != nil {
				if err := canaryBuffer.Add(lastStreamData); err != nil {
					logger.LogError(c, "anti-poison canary stream parse failed: "+err.Error())
					sr.Stop(err)
					return
				}
		...
		if len(data) > 0 {
			if info.AntiPoisonGuardPrefix != "" {
				cfg := antipoison.ResponseGuardConfig(info)
				cleanData, markers, tools, cleanErr := antipoison.ValidateAndStripOpenAIStreamChunk(data, cfg, info.AntiPoisonGuardPrefix)
				if cleanErr != nil {
					logger.LogError(c, "anti-poison stream parse failed: "+cleanErr.Error())
					sr.Error(cleanErr)
				} else {
					data = cleanData
					antiPoisonMarkers = append(antiPoisonMarkers, markers...)
					antiPoisonTools = append(antiPoisonTools, tools...)
				}
			}
			if responseProof != nil && !responseProof.Verified() {
				cleanData, hold, proofErr := stripOpenAIStreamResponseProof(data, responseProof)
				if proofErr != nil {
					logger.LogError(c, "anti-poison response proof stream validation failed: "+proofErr.Error())
					sr.Stop(proofErr)
					return
				}
				if hold {
					if cleanData != "" {
						pendingStreamData = append(pendingStreamData, cleanData)
					}
					return
				}
				data = cleanData
	...
	if info.AntiPoisonGuardPrefix != "" {
		...
		if err := antipoison.ValidateOpenAIStreamFinal(antiPoisonMarkers, antiPoisonTools, cfg, info.AntiPoisonGuardPrefix); err != nil {
			logger.LogError(c, "anti-poison stream validation failed: "+err.Error())
	...
	if canaryBuffer != nil {
		...
		cleanChunks, err := canaryBuffer.Finalize()
		if err != nil {
			common.SetContextKey(c, constant.ContextKeyAntiPoisonEvidenceResponse, canaryBuffer.RawText())
			logger.LogError(c, "anti-poison canary stream validation failed: "+err.Error())
			return nil, types.NewError(err, types.ErrorCodeAntiPoisonValidationFailed)
		}
		if info.RelayFormat == types.RelayFormatOpenAI && !shouldSendLastResp && len(cleanChunks) > 0 {
			cleanChunks = cleanChunks[:len(cleanChunks)-1]
		}
		for _, cleanData := range cleanChunks {
			if err := HandleStreamFormat(c, info, cleanData, info.ChannelSetting.ForceFormat, info.ChannelSetting.ThinkingToContent); err != nil {
				logger.LogError(c, "error replaying canary-validated stream: "+err.Error())
		...
		if responseProof != nil {
			if err := responseProof.Finalize(); err != nil {
				logger.LogError(c, "anti-poison response proof stream validation failed: "+err.Error())
